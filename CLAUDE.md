...
* AI call log dialog scroll fixed: `.log-dialog` is a flex column with
  `overflow: hidden` (the `display: flex` is on `.log-dialog[open]` only — a bare
  `.log-dialog { display: flex }` overrides the UA `dialog:not([open])` hide and
  the dialog never closes), and `.log-scroll` is `flex: 1; min-height: 0` (its
  old `max-height: 66vh` removed) — header/blurb stay put, inner table is the
  only scroller. `body:has(dialog[open]) { overflow: hidden }` also freezes the
  page behind any open modal.
